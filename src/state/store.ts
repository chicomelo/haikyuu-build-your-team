...
const emptyTeam: Team = {
  slots: { S: null, MB1: null, WS1: null, L: null, WS2: null, MB2: null, OP: null },
  bench: [],
}
...
function ensureBenchSize(arr: string[], size = 18) {
...
export const useStore = create<Store>((set, get) => ({
  ...
  loadTeam: (newTeam, newRotationLabels) => set(() => {
    // Garantir que a bancada tenha exatamente 18 slots
    const bench = [...newTeam.bench];
    while (bench.length < 18) bench.push("");
    if (bench.length > 18) bench.splice(18);
    
    // Verificar se newRotationLabels está presente e é válido
    const validRotationLabels = newRotationLabels && Object.keys(newRotationLabels).length > 0 
      ? newRotationLabels 
  ...
  clearPosition: (pos) => set((s) => ({ team: { ...s.team, slots: { ...s.team.slots, [pos]: null } } })),

  placePlayer: (pos, playerId) => set((s) => {
  ...
  moveToBench: (playerId, benchIndex) => set((s) => {
    const slots = { ...s.team.slots }
    // Remove from wherever it is (slots and bench)
    for (const k in slots) {
      const key = k as Position
  ...
  removeFromBench: (playerId) => set((s) => {
    const bench = ensureBenchSize(s.team.bench)
